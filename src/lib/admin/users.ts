...
export type AdminUserRow = {
  ...
  phone: string;
  location: string;
  website: string;      // profile website, else the first of their listings' websites
  isAdmin: boolean;
  listings: number;
...
export async function getAdminUsers(
  ...
  const [{ data: profiles }, { data: cats }, { data: listings }, authRes] = await Promise.all([
    admin.from('profiles').select('id, full_name, phone, city, country, website, is_admin, created_at'),
    admin.from('categories').select('id, name, slug, parent_id'),
    admin.from('listings').select('owner_id, category_id, website'),
  ...
  const listingAgg = new Map<string, { n: number; names: Set<string>; slugs: Set<string>; sites: Set<string> }>();
  for (const l of (listings ?? []) as any[]) {
    const agg = listingAgg.get(l.owner_id) ?? { n: 0, names: new Set(), slugs: new Set(), sites: new Set() };
    agg.n += 1;
    const nm = motherName(l.category_id);
    const sl = motherSlug(l.category_id);
    if (nm) agg.names.add(nm);
    if (sl) agg.slugs.add(sl);
    if (l.website) agg.sites.add(l.website);
  ...
  let rows: AdminUserRow[] = ((profiles ?? []) as any[]).map((p) => {
    ...
    return {
      ...
      website: p.website || (agg ? [...agg.sites][0] ?? '' : ''),
      isAdmin: !!p.is_admin,
      listings: agg?.n ?? 0,
